...
export function ConfirmReceiptClient({
  ...
  return (
    <div className="flex-1 bg-brand-950 px-5 py-6">
      ...
      <h1 className="font-sans font-bold text-2xl text-onbrand mb-1">Confirme a compra</h1>
      <p className="text-sm text-onbrand/55 mb-5">Confira os dados que o Tobias leu da nota antes de salvar.</p>

      {lowConfidence && (
        <div className="flex items-start gap-2 rounded-xl bg-warn-100/10 border border-warn-600/40 p-3 mb-5 text-sm text-onbrand/80">
        ...
        {items.length > 0 && (
          <Card>
            <CardContent className="py-3">
              <p className="text-xs font-medium text-onbrand/55 mb-2">Itens identificados</p>
              <ul className="space-y-1.5">
                {items.map((it, i) => (
                  <li key={i} className="flex justify-between text-sm text-onbrand/80">
