...
export default async function ActivityPage() {
  ...
  return (
    <div className="flex-1 bg-brand-950 px-5 py-6">
      ...
      <Link href="/settings" className="inline-flex items-center gap-1.5 text-sm text-onbrand/55 hover:text-gold-400 mb-4">
        <ArrowLeft className="h-3.5 w-3.5" /> Configurações
      </Link>
      <h1 className="font-sans font-bold text-2xl text-onbrand mb-1">Atividade recente</h1>
          ...
          <CardContent className="py-2 divide-y divide-white/10">
            {events.map((e) => (
              <div key={e.id} className="py-3 flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-onbrand/40 shrink-0">
