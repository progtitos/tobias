...
export const AIService = {
  ...
  async extractStatement(
    ...
    return generateVisionJSON({
      system: `${TOBIAS_PERSONA}\n\nVocê está extraindo as transações de ${framing} Retorne TODAS as linhas de movimentação que conseguir ler, na ordem em que aparecem. Nunca invente uma transação que não está no documento, e nunca invente um número de parcela (ex: "2/12") que o documento não mostrar explicitamente — se não tiver certeza, deixe installmentNumber/installmentTotal como null. Se houver várias páginas/fotos, consolide como um único extrato contínuo, sem repetir transações que aparecem em mais de uma página. NÃO inclua linhas informativas que não são uma movimentação de verdade (ex: "saldo restante da fatura anterior", "limite disponível", "saldo em conta") — elas não têm um valor de transação de fato e devem ser ignoradas, mesmo que apareçam impressas no documento. IMPORTANTE sobre datas: muitas faturas mostram cada linha só como "DD/MM", sem o ano. Nesses casos, NUNCA devolva a data sem ano — descubra o ano correto a partir do período da fatura (mês de referência, data de fechamento/vencimento, ou qualquer outra indicação de ano no documento) e devolva sempre "AAAA-MM-DD" completo.`,
      message:
